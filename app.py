...
# Create new route for the training page
@app.route('/training')
# User must be logged in to view this route
@login_required
# Only the ADMIN and the MANAGER is able to access this route
@role_required(['ADMIN', 'MANAGER'])
def training():
    # If the user is either a MANAGER or an ADMIN then they can view the training template
    if current_user.role in ['MANAGER', 'ADMIN']:
        return render_template('training.html')
    # If the user is not able to access this page show the error
    else:
        flash(f'Invalid request. Please contact support if this problem persists.', 'error')
        return render_template('error.html')
